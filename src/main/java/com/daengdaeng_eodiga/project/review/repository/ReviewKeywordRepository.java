...
public interface ReviewKeywordRepository extends JpaRepository<ReviewKeyword, Integer> {
	...
	@Query(value = "SELECT rk.id.keyword FROM Review r LEFT JOIN FETCH ReviewKeyword rk ON rk.id.reviewId = r.reviewId WHERE r.place.placeId = :placeId GROUP BY rk.id.keyword ORDER BY COUNT(rk.id.keyword) DESC")
